...
def solve(grid):
    # On va créer une methode search qui va faire une recherche en profondeur
    # dans l'arbre des solutions possibles
    # Cette fonction va prendre en paramètre des values au stade terminal de la
    # propagation des contraintes 1 et 2

def search(values):
    """ Utilisation de la recherche en profondeur et de la propagation,
    on essaye toutes les valeurs possibles."""

    # Si la grille n'est pas solvable, on ne va pas plus loin et on va renvoyer
    # False

    # Si toutes les cases de la grille n'ont qu'une seule valeur possible alors
    # la grille est résolue, on s'arrête et on renvoit la grille

    # On choisit la case à modifier : on prend la case qui a la plus petite
    # longeur de chiffres possibles

    # On copie le vecteur values

    # Pour toutes les valeurs d de s
        # On assigne à la case s la valeur d
        # On itère la recherche
        # si la recherche retourne False => continue
        # Si la recherche retourne values => on continue avec search


def some(seq):
    " Renvoit un élément de la séquence qui est True. "
    for e in seq:
        if e: 
            return e
    return False
...
def isSolved(values):
    ...
    1 à 9. """
    # On va définie une fonction qui prend en paramètre un unit et qui renvoie
    # True si l'unité est complète (ie le set de l'unité est égal au set des
    # digits

    # en utilisant cette fonction valider toutes les unités de la liste en
    # s'assurant que values n'est pas False

def time_solve(grid):
    """ Fonction permettant de mesurer le temps mis pour résoudre un problème"""
    # utilisation de time.clock() pour récupérer l'heure
    # résolution
